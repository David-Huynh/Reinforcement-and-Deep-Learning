...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = None
        # ========================================================
        # TODO: Compute forward call for a transformer encoder
        # The shapes of relevant variables:
        # - x: (batch_size, context_length, embed_dim)
        x = x.to(torch.float32)

        # Step 1: LayerNorm  Self-Attention  Residual
        xnorm1 = self.ln_1(x)
        attn, _ = self.attention(xnorm1, xnorm1, xnorm1)
        x = x + attn

        # Step 2: LayerNorm  MLP  Residual
        xnorm2 = self.ln_2(x)
        mlp_out = self.mlp_2(torch.nn.functional.gelu(self.mlp_1(xnorm2)))
        x = x + mlp_out
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, cls_token=None):
        ...
        # Add the [CLS] token to the tokens
        tokens = torch.cat([cls_token, tokens], dim=1)

        # Add positional encodings to the tokens
        tokens = tokens + self.pos_encoding[:, :tokens.size(1), :]
        
        # Pass the tokens through the transformer encoder blocks
