...
class MultiHeadAttention:
    ...
    def __call__(self, q, k, v):
        ...
        k = k @ self.params_k
        v = v @ self.params_v
        # q, k, v are shaped (batch_size, context_length, embed_dim)

        # Step 2: Reshape for multihead attention
        q = q.reshape(batch_size, context_length, self.num_heads, head_dim).transpose(0, 2, 1, 3)
        k = k.reshape(batch_size, context_length, self.num_heads, head_dim).transpose(0, 2, 1, 3)
        v = v.reshape(batch_size, context_length, self.num_heads, head_dim).transpose(0, 2, 1, 3)

        # q, k, v are shaped (batch_size, num_heads, context_length, head_dim)

        # Step 3: Apply softmax attention
        attention_scores = q @ k.transpose(0, 1, 3, 2) * scale
        attention_weights = softmax(attention_scores)  # Use PyTorch's softmax for consistency
        out = attention_weights @ v
        # out is shaped (batch_size, num_heads, context_length, head_dim)

        out = out.transpose(0, 2, 1, 3).reshape(batch_size, context_length, embed_dim)
