...
def compute_gradient_penalty(model, real_images, fake_images, eps):
    ...
    preds = model.discriminate(interpolated_images)

    # Compute gradients of discriminator output w.r.t. interpolated images
    gradients = autograd.grad(
        outputs=preds,
        inputs=interpolated_images,
        grad_outputs=torch.ones_like(preds, device="cpu"),
        create_graph=True,
        retain_graph=True,
        only_inputs=True,
        allow_unused=True
    ...
    gp_loss = ((gradient_norm - 1) ** 2).mean()  # Gradient penalty loss
...
def train_step_wgan(batch, model, optimizer, gp):
    ...
    disc_loss = (real_preds.mean() - fake_preds.mean())
    # ========================================================
